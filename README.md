# Year2035
Kこの作品の核心をなす要素とプロットは以下の通りです：


### 要件:
- **設定**: 2035年のテクノロジーが発展し、デジタル監視が日常化した東京。
- **造語/概念**:
...
### プロット:
1. **導入**: 東京がテクノロジーと監視の下で変化した未来世界を舞台に設定。
2. **展開**: ハルコとユウキは、政府の抑圧に対抗するために非合法の活動に参加し、無名者マーケットを利用して情報を共有する。
3. **クライマックス**: ユウキは政府のホログラム言明システムをハイジャックし、抑圧された真実を市民に伝える。
4. **結末**: その行動が市民の間で共感と反響を呼び、抵抗運動が拡大する一方、ハルコとユウキは捕えられるが、彼らのメッセージは残り、社会に変化のきっかけをもたらす。
